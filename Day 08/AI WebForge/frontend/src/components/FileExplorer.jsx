...
function TreeItem({ node, activeFile, onFileSelect, depth = 0 }) {
  ...
  function getFileIcon(name) {
    if (name.endsWith(".css")) {
      return <FileTextIcon size={14} className="text-sky-700" />;
    }

    if (name.endsWith(".jsx") || name.endsWith(".js")) {
      return <FileCodeIcon size={14} className="text-amber-700" />;
    }

    if (name.endsWith(".json")) {
      return <FileTextIcon size={14} className="text-emerald-700" />;
    }
